...
pub fn add_watcher(
    ...
    {
        // No `MSG_DONTWAIT` here: make drain reads non-blocking instead.
        // See `ensure_nonblocking` for the caveat.
        ensure_nonblocking(newfd).map_err(PyErr::from)?;
    }
...
/// Per-call non-blocking flag for drain reads. Linux has `MSG_DONTWAIT`;
/// other unixes set `O_NONBLOCK` on our owned duplicate at registration
/// instead (see `ensure_nonblocking`).
#[cfg(target_os = "linux")]
const RECV_FLAGS: libc::c_int = libc::MSG_DONTWAIT;
#[cfg(not(target_os = "linux"))]
const RECV_FLAGS: libc::c_int = 0;

/// Mark an owned fd non-blocking (non-Linux unixes only, where the drain
/// reads cannot use `MSG_DONTWAIT`). Only ever called on our `dup`'d
/// descriptor. Note the flag lives on the shared file description, so on
/// these platforms registering a transport also flips the user's socket to
/// non-blocking — matching what asyncio transports require anyway.
#[cfg(all(unix, not(target_os = "linux")))]
fn ensure_nonblocking(fd: RawFd) -> std::io::Result<()> {
    // SAFETY: `fcntl` with `F_GETFL`/`F_SETFL` on a valid owned fd.
...
/// Non-blocking stream read. On Linux `MSG_DONTWAIT` (not the socket flags)
/// is used deliberately: a blocking read here would freeze the whole event
/// loop, and this way registration never mutates user-visible socket flags.
fn tcp_recv(dup_fd: RawFd, buf: &mut [u8]) -> std::io::Result<usize> {
    loop {
        ...
        // non-blocking is guaranteed by `RECV_FLAGS` / `ensure_nonblocking`.
        let r = unsafe {
            libc::recv(
                dup_fd,
...
fn parse_addr(storage: &libc::sockaddr_storage, len: usize) -> AddrRepr {    match storage.ss_family as libc::c_int {
        ...
        libc::AF_INET6 => {
            ...
            let ip = std::net::Ipv6Addr::from(in6_addr_bytes(&a.sin6_addr));
